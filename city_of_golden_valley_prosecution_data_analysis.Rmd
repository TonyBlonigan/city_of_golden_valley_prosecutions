...
```{r low_level_driving_offenses_by_race_fisher_exact_test, echo=FALSE}
# 
...
```{r, echo=FALSE, warning=FALSE, fig.height=6.25}
...
ggplot(p, aes(name, value, fill=race)) + 
  ...
  coord_flip() +
  guides(fill = "none")+
  labs(x = element_blank(), y = 'Contribution to Evidence that Disposition is Not Independent of Race')
...
```{r low_level_driving_offenses_by_race_chi_square, echo=FALSE, include=FALSE}
chisq = chisq.test(low_level_driving_offenses_by_race)
...
Comparing the racial composition of the low level driving offenses to the [American Communities Survey](https://stats.metc.state.mn.us/profile/detail.aspx?c=02394924) demographic data (ACS) for the Golden Valley, it is immediately apparent that the black population is prosecuted at a higher rate than the white population. The racial/ethnic groups represented in the CC prosecution data set are so different from the city's population that statistical analysis is hardly required to detect the relationship between race/ethnicity and prosecution for low-level driving offences. That said, below is a statistical analysis supporting this conclusion.
...
While keeping these limitations in mind, it is still possible to use statistical tools to help us quantify our certainty that prosecution for low level driving offences is associated with race/ethnicity.
