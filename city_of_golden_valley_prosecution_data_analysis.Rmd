...
Comparing the racial composition of the low level driving offenses to the [American Communities Survey](https://stats.metc.state.mn.us/profile/detail.aspx?c=02394924) demographic data (ACS) for the Golden Valley, it is immediately apparent that the black population is represented at a much higher rate than you would expect, based on Golden Valley's demographics. 

Unfortunately, there are several limitations to the available data preventing us from doing a proper statistical analysis, such as:
...
While keeping these limitations in mind, it is still possible to use statistical tools to help us understand how prosecution for low level driving offences appear to be related to race/ethnicity.
...
In the absence of the data integrity issues enumerated, above, the chi-squared test of independence could be used to asses whether prosecution is associated with race/ethnicity. While we can't use this as a statistical test to determine whehter there is a relationship between race and prosecution for low level driving offences, we can still use this test to hilight some concerning areas in the data. 
...
Below, is a plot that illustrates the difference between the racial/ethnic distribution of prosecutions and the racial/ethnic distribution of Golden Valley, quantified as Pearson Residuals. The lenght of the bars correspond to the size of the difference between the prosecution and Golden Valley populations.
